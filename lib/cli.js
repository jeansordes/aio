...
SYNOPSIS
     ...
     aio init
     aio setup
     aio run [ --quiet | -q ] [ --verbose | -v ] [ --loops N | -n N ]
             [ --allow-edits-outside-dir ] [ <workflow> ] [ <loops> ]
...
COMMANDS
     ...
     run [ flags ] [ <workflow> ] [ <loops> ]
             Run a workflow. The default <workflow> name is default, i.e. the
             file .aio/workflows/default.yaml. Optional <loops> is a
             non-negative integer: 1 (default), N>0 for that many independent
             runs, or 0 for infinite loops until SIGINT. The same count can be
             set with --loops or -n (do not combine with a second positional
             loop count). Each loop uses a fresh run id and log directory under
             .aio/runs/. First SIGINT finishes the current loop; a second
             SIGINT aborts the in-flight provider. Live provider output is
...
function parseRunArgs(args) {
  ...
  for (let i = 0; i < args.length; i += 1) {
    ...
    if (a === "--quiet" || a === "-q") {
      ...
      if (!next || next.startsWith("-")) {
        throw new Error("aio: --loops / -n requires a non-negative integer");
      }
      const n = Number.parseInt(next, 10);
      if (!Number.isInteger(n) || n < 0) {
        throw new Error("aio: --loops / -n must be a non-negative integer");
      }
      loopsFromFlag = n;
      i += 1;
  ...
  if (loopsFromFlag !== undefined) {
    if (positional.length > 1) {
      throw new Error("aio: with --loops / -n, pass at most one workflow name");
    }
    if (positional.length === 1 && /^\d+$/.test(positional[0])) {
      throw new Error("aio: do not pass a numeric loop count as a positional argument when using --loops / -n");
    }
    workflowName = positional[0] ?? "default";
    loops = loopsFromFlag;
    ...
    if (!Number.isInteger(n) || n < 0) {
      throw new Error("aio: second positional argument must be a non-negative integer (loop count)");
    }
    loops = n;
  } else {
  ...
  return { quiet, verbose, allowEditsOutsideDir, workflowName, loops };
}
...
async function routeCommand(argv, options = {}) {
  ...
  if (command === "run") {
    const { quiet, verbose, allowEditsOutsideDir, workflowName, loops } = parseRunArgs(argv.slice(1));
    const stdout = options.stdout ?? process.stdout;
    ...
    const onSigint = () => {
      if (!stopRequested) {
        stopRequested = true;
        stderr.write(
          "\naio: stop requested; finishing current loop. Press Ctrl-C again to abort the in-flight provider.\n",
        );
      } else {
    ...
    try {
      for (let i = 0; loops === 0 || i < loops; i += 1) {
        const bus = createEventBus();
        currentBus = bus;
        bus.emit("loopStart", { iteration: i + 1, total: loops || null });
        ...
        bus.emit("loopEnd", { iteration: i + 1 });
        if (stopRequested) {
          break;
        }
      }
