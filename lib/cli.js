...
SYNOPSIS
     ...
     aio init
     aio setup
     aio run [ --quiet | -q ] [ --verbose | -v ] [ <workflow> ]
...
COMMANDS
     ...
     run [ flags ] [ <workflow> ]
             ...
             .aio/workflows/<workflow>.yaml. Live provider output is streamed
             to the terminal; --quiet limits output to milestones; --verbose is
             reserved for noisier diagnostics. Each run writes
...
function parseRunArgs(args) {
  ...
  for (const a of args) {
    if (a === "--quiet" || a === "-q") {
      ...
      /* reserved for noisier diagnostics */
  ...
  return { quiet, workflowName: positional[0] ?? "default" };
}
...
async function routeCommand(argv, options = {}) {
  ...
  if (command === "run") {
    const { quiet, workflowName } = parseRunArgs(argv.slice(1));
    const bus = createEventBus();
    const { close: closeRunLog } = startRunLog(projectRoot, {
      bus,
