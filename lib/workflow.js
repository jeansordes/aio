...
const DEFAULT_SUMMARY_PROMPT =
  "In 1-3 sentences, summarize what you accomplished in this workflow step and any notable outcomes.";

/**
 * @param {object} state
 * @returns {{ enabled: boolean, prompt: string | null } | null}
 */
function parseStateSummary(state) {
  if (state.summary == null) {
    return null;
  }
  if (state.summary === true) {
    return { enabled: true, prompt: null };
  }
  if (typeof state.summary === "object" && !Array.isArray(state.summary)) {
    return {
      enabled: state.summary.enabled === true,
      prompt: typeof state.summary.prompt === "string" ? state.summary.prompt : null,
    };
  }
  return null;
}

function buildSummaryPrompt({ stateName, roleName, customPrompt }) {
  const core = (customPrompt && customPrompt.trim()) || DEFAULT_SUMMARY_PROMPT;
  return [
    "You have just finished a workflow step.",
    `State: ${stateName}`,
    `Role: ${roleName}`,
    "",
    core,
...
async function runWorkflow(options = {}) {
  ...
  try {
    for (let step = 0; step < maxSteps; step += 1) {
      ...
      const summaryCfg = parseStateSummary(state);
      if (summaryCfg?.enabled && providerOutput.step.status === "ok") {
        const summaryPrompt = buildSummaryPrompt({
          stateName: current,
          roleName: state.role,
          customPrompt: summaryCfg.prompt,
        });
        const summaryOut = await executeProvider({
          projectRoot,
          workflow: workflowLabel,
          stateName: current,
          roleName: state.role,
        ...
        summaryText = (summaryOut.stdout ?? "").trim();
        if (!summaryText && summaryOut.stderr) {
          summaryText = summaryOut.stderr.trim();
        }
        bus?.emit?.("summary", { stateName: current, roleName: state.role, text: summaryText });
      }
...
function buildPrompt({ workflow, stateName, roleName, role, git, projectKnowledge, promptConfig }) {
