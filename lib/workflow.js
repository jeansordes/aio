...
function executeProvider({ projectRoot, workflow, stateName, roleName, role, git, projectKnowledge, providersConfig }) {
  ...
  if (!provider || provider.status === "not_configured" || !provider.command) {
    ...
    return providerResult({
      before,
      after: getWorkingTreeSnapshot(projectRoot),
      stdout: message,
      stderr: "",
      exitCode: null,
  ...
  const result = spawnSync(provider.command, args, {
    cwd: projectRoot,
    encoding: "utf8",
    input: "",
    shell: false,
  ...
  const status = okExitCodes.includes(result.status) ? "ok" : "failed";
  return providerResult({
    before,
    after: getWorkingTreeSnapshot(projectRoot),
