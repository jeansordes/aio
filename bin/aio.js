...
async function main() {
  ...
  if (shouldOfferUpdate(latestVersion, { installContext })) {
    const accepted = await promptForUpdate(latestVersion);
    if (accepted) {
...
function shouldOfferUpdate(latestVersion, options = {}) {
  ...
  if (installContext !== "global-npm" && installContext !== "global-bun") {
    return false;
  }
...
function compareVersions(left, right) {
  const leftParts = left.split(".").map(toSafeNumber);
  const rightParts = right.split(".").map(toSafeNumber);
...
function promptForUpdate(latestVersion) {
  return new Promise((resolve) => {
    ...
    cli.question(
      `A newer version of ${PACKAGE_NAME} is available (${CURRENT_VERSION} -> ${latestVersion}). Update now? (y/n) `,
      (answer) => {
        cli.close();
...
function detectInstallContext(options = {}) {
  ...
  const scriptPath = resolvePathOrNull(options.scriptPath ?? __filename);
  const packageRoot = resolvePackageRoot(scriptPath);
  const argv = options.argv ?? process.argv;
  const argv0 = options.argv0 ?? process.argv0;
  const env = options.env ?? process.env;
  const npmGlobalRoot = normalizeDirectoryPath(options.npmGlobalRoot ?? getNpmGlobalRoot());
  ...
  if (packageRoot && npmGlobalRoot && isSubpath(packageRoot, npmGlobalRoot)) {
    return "global-npm";
  }

  if (invocationPath && bunGlobalBin && isSubpath(invocationPath, bunGlobalBin)) {
    return "global-bun";
  }
...
function isNpxExecution({ argv0, env }) {
  ...
  return basenameMatches(argv0, "npx") || userAgent.startsWith("npm/") || execPath.includes("npm-cli");
}
